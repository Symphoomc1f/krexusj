 ...
 **/
@ServerEndpoint("/openDoorMonitor/{clientId}/{machineId}")
@Component
public class OpenDoorMonitorWebSocketServer {

    private static Logger logger = LoggerFactory.getLogger(OpenDoorMonitorWebSocketServer.class);
     ...
     */
    private static ConcurrentHashMap<String, OpenDoorMonitorWebSocketServer> webSocketMap = new ConcurrentHashMap<>();
     ...
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("clientId") String clientId, @PathParam("machineId") String machineId) {
     ...
     * 发送设备监控信息
     */
    public static void sendInfo(String message, String machineId) throws IOException {
        logger.info("发送消息到:" + machineId + "，报文:" + message);
        for(OpenDoorMonitorWebSocketServer server : webSocketMap.values()){
            if(machineId.equals(server.machineId)){
    ...
    public static synchronized void addOnlineCount() {
        OpenDoorMonitorWebSocketServer.onlineCount++;
    }
